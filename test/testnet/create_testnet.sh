...
mkdir -p tmp
export testnet_datadir=$(mktemp -d "tmp/XXXX")
...
function start {
  ...
  ./start.sh "$testnet_datadir"
}
#create config.json
echo First run, OK to error about a missing RPC password...
start||true
sed -i 's/"rpc_user": ""/"rpc_user": "test"/' ${testnet_datadir}/config.json
sed -i 's/"rpc_password": ""/"rpc_password": "test"/' ${testnet_datadir}/config.json
sed -i 's#./htdocs#'"$WEB_WALLET_GEN"'#' ${testnet_datadir}/config.json
echo
...
echo Start this server as follows:
echo "./start.sh $testnet_datadir"
